...
/*   Updated: 2020/09/18 13:52:01 by lnoaille         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_extract_color(char *line, t_img *param, char fc)
{
	...
	if (!(ft_is_rgb(line)))
		return (0);
	i = 0;
	r = ft_atoi(&line[i]) > 255 ? 255 : ft_atoi(&line[i]);
	if (!(i = ft_is_nb(i, line)))
		return (0);
	if (line[i] == ',')
		i++;
	g = ft_atoi(&line[i]) > 255 ? 255 : ft_atoi(&line[i]);
	if (!(i = ft_is_nb(i, line)))
		return (0);
	if (line[i] == ',')
		i++;
	b = ft_atoi(&line[i]) > 255 ? 255 : ft_atoi(&line[i]);
